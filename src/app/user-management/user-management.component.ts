...
export class UserManagementComponent implements OnInit {
  ...
  prepareGrid() {
    this.columnDefinitions = [
      {
        id: 'name', name: 'User Name', field: 'name', sortable: true, type: FieldType.string, maxWidth: 200,
        filterable: true,
      ...
      },
      {
        ...
        editor: {
          ...
          collection: [{ value: 'Admin', label: 'Admin' },
      ...
      {
        id: 'type', name: 'Type', field: 'type', maxWidth: 180,
  ...
  openCompositeModal(modalType: CompositeEditorModalType) {
    ...
    switch (modalType) {
      case 'create':
        modalTitle = 'Creating New User';
        break;
      case 'clone':
        modalTitle = 'Clone - {{name}}';
        break;
      case 'edit':
        modalTitle = 'Editing User - {{name}}'; // 'Editing - {{title}} ({{product.itemName}})'
        break;
      case 'mass-update':
        ...
        if (modalType === 'mass-update' || modalType === 'mass-selection') {
          ...
          if (modalType === 'mass-update') {
            this.angularGrid.dataView.getItems().forEach(row => {
              users.push(Object.assign(row, formValues));
            });
          } else {
          ...
          return this.apiService.updateUsers(users).toPromise();
        } else {
